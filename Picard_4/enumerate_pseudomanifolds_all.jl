...
function build_finalDB_single_v!(pseudo_manifolds_DB::Dict{Int,Vector{Set{BitVector}}},mat_DB::Dict{Int,Vector{Vector{UInt16}}},iso_DB::Dict{Int,Dict{Int,Vector{Tuple{Int,Any}}}},mmax;mstart=-1,last_single_link=false)
        ...
        for (l,bases_bin) in enumerate(mat_DB[m])
            ...
            if m==mmin # Base case
                ...
                for (index_contraction, perm) in iso_DB[m][l] # find by computing the links
                    ...
                    if m==mmax && last_single_link# In the last case, no need to compute all links
                        break
                    end
                end
            end
